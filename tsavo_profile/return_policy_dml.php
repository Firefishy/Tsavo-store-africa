...
// Data functions (insert, update, delete, form) for table return_policy

// This script and data application were generated by AppGini 5.96
// Download AppGini for free from https://bigprof.com/appgini/download/

function return_policy_insert(&$error_message = '') {
	global $Translation;

	// mm: can member insert record?
	$arrPerm = getTablePermissions('return_policy');
	...
	// hook: return_policy_before_insert
	if(function_exists('return_policy_before_insert')) {
		$args = [];
		if(!return_policy_before_insert($data, getMemberInfo(), $args)) {
	...
	insert('return_policy', backtick_keys_once($data), $error);
	if($error)
		die("{$error}<br><a href=\"#\" onclick=\"history.go(-1);\">{$Translation['< back']}</a>");

	$recID = db_insert_id(db_link());

	update_calc_fields('return_policy', $recID, calculated_fields()['return_policy']);

	// hook: return_policy_after_insert
	if(function_exists('return_policy_after_insert')) {
		$res = sql("SELECT * FROM `return_policy` WHERE `id`='" . makeSafe($recID, false) . "' LIMIT 1", $eo);
		if($row = db_fetch_assoc($res)) {
			$data = array_map('makeSafe', $row);
		}
		$data['selectedID'] = makeSafe($recID, false);
		$args=[];
		if(!return_policy_after_insert($data, getMemberInfo(), $args)) { return $recID; }
	}

	// mm: save ownership data
	set_record_owner('return_policy', $recID, getLoggedMemberID());

	// if this record is a copy of another record, copy children if applicable
	if(!empty($_REQUEST['SelectedID'])) return_policy_copy_children($recID, $_REQUEST['SelectedID']);

	return $recID;
}

function return_policy_copy_children($destination_id, $source_id) {
	global $Translation;
...
function return_policy_delete($selected_id, $AllowDeleteOfParents = false, $skipChecks = false) {
	// insure referential integrity ...
	global $Translation;
	$selected_id = makeSafe($selected_id);

	// mm: can member delete record?
	if(!check_record_permission('return_policy', $selected_id, 'delete')) {
		return $Translation['You don\'t have enough permissions to delete this record'];
	}

	// hook: return_policy_before_delete
	if(function_exists('return_policy_before_delete')) {
		$args = [];
		if(!return_policy_before_delete($selected_id, $skipChecks, getMemberInfo(), $args))
	...
	sql("DELETE FROM `return_policy` WHERE `id`='{$selected_id}'", $eo);

	// hook: return_policy_after_delete
	if(function_exists('return_policy_after_delete')) {
		$args = [];
		return_policy_after_delete($selected_id, getMemberInfo(), $args);
	}

	// mm: delete ownership data
	sql("DELETE FROM `membership_userrecords` WHERE `tableName`='return_policy' AND `pkValue`='{$selected_id}'", $eo);
}

function return_policy_update(&$selected_id, &$error_message = '') {
	global $Translation;

	// mm: can member edit record?
	if(!check_record_permission('return_policy', $selected_id, 'edit')) return false;
	...
	// get existing values
	$old_data = getRecord('return_policy', $selected_id);
	...
	// hook: return_policy_before_update
	if(function_exists('return_policy_before_update')) {
		$args = ['old_data' => $old_data];
		if(!return_policy_before_update($data, getMemberInfo(), $args)) {
			if(isset($args['error_message'])) $error_message = $args['error_message'];
			return false;
		}
	}
	...
	if(!update(
		...
		echo $error_message;
		echo '<a href="return_policy_view.php?SelectedID=' . urlencode($selected_id) . "\">{$Translation['< back']}</a>";
		exit;
	}
	...
	update_calc_fields('return_policy', $data['selectedID'], calculated_fields()['return_policy']);

	// hook: return_policy_after_update
	if(function_exists('return_policy_after_update')) {
		$res = sql("SELECT * FROM `return_policy` WHERE `id`='{$data['selectedID']}' LIMIT 1", $eo);
		if($row = db_fetch_assoc($res)) $data = array_map('makeSafe', $row);

		$data['selectedID'] = $data['id'];
		$args = ['old_data' => $old_data];
		if(!return_policy_after_update($data, getMemberInfo(), $args)) return;
	}

	// mm: update ownership data
	sql("UPDATE `membership_userrecords` SET `dateUpdated`='" . time() . "' WHERE `tableName`='return_policy' AND `pkValue`='" . makeSafe($selected_id) . "'", $eo);
}

function return_policy_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0, $TemplateDV = '', $TemplateDVP = '') {
	...
	if($selected_id) {
		...
		// mm: who is the owner?
		$ownerGroupID = sqlValue("SELECT `groupID` FROM `membership_userrecords` WHERE `tableName`='return_policy' AND `pkValue`='" . makeSafe($selected_id) . "'");
		$ownerMemberID = sqlValue("SELECT LCASE(`memberID`) FROM `membership_userrecords` WHERE `tableName`='return_policy' AND `pkValue`='" . makeSafe($selected_id) . "'");
		...
		$res = sql("SELECT * FROM `return_policy` WHERE `id`='" . makeSafe($selected_id) . "'", $eo);
		if(!($row = db_fetch_array($res))) {
			return error_message($Translation['No records found'], 'return_policy_view.php', false);
		}
	...
	if($dvprint) {
		$template_file = is_file("./{$TemplateDVP}") ? "./{$TemplateDVP}" : './templates/return_policy_templateDVP.html';
		$templateCode = @file_get_contents($template_file);
	} else {
		$template_file = is_file("./{$TemplateDV}") ? "./{$TemplateDV}" : './templates/return_policy_templateDV.html';
		$templateCode = @file_get_contents($template_file);
	}
	...
	// process buttons
	if($AllowInsert) {
		if(!$selected_id) $templateCode = str_replace('<%%INSERT_BUTTON%%>', '<button type="submit" class="btn btn-success" id="insert" name="insert_x" value="1" onclick="return return_policy_validateData();"><i class="glyphicon glyphicon-plus-sign"></i> ' . $Translation['Save New'] . '</button>', $templateCode);
		$templateCode = str_replace('<%%INSERT_BUTTON%%>', '<button type="submit" class="btn btn-default" id="insert" name="insert_x" value="1" onclick="return return_policy_validateData();"><i class="glyphicon glyphicon-plus-sign"></i> ' . $Translation['Save As Copy'] . '</button>', $templateCode);
	} else {
	...
	if($selected_id) {
		...
		if($AllowUpdate) {
			$templateCode = str_replace('<%%UPDATE_BUTTON%%>', '<button type="submit" class="btn btn-success btn-lg" id="update" name="update_x" value="1" onclick="return return_policy_validateData();" title="' . html_attr($Translation['Save Changes']) . '"><i class="glyphicon glyphicon-ok"></i> ' . $Translation['Save Changes'] . '</button>', $templateCode);
		} else {
	...
	$rdata = $jdata = get_defaults('return_policy');
	if($selected_id) {
		$jdata = get_joined_record('return_policy', $selected_id);
		if($jdata === false) $jdata = get_defaults('return_policy');
		$rdata = $row;
	}
	$templateCode .= loadView('return_policy-ajax-cache', array('rdata' => $rdata, 'jdata' => $jdata));

	// hook: return_policy_dv
	if(function_exists('return_policy_dv')) {
		$args=[];
		return_policy_dv(($selected_id ? $selected_id : FALSE), getMemberInfo(), $templateCode, $args);
	}
