	...
	*/
	...
	function getTableList($skip_authentication = false) {
		...
		$arrTables = [
			...
			'system_setting' => ['System_setting', '', 'resources/table_icons/table.png', 'None'],
			'supplier' => ['Supplier', '', 'resources/table_icons/client_account_template.png', 'None'],
			'products' => ['Products', '', 'table.gif', 'None'],
			'details' => ['Details', '', 'resources/table_icons/card_money.png', 'None'],
			'category' => ['Category', '', 'resources/table_icons/photos.png', 'None'],
		];
	...
	function get_sql_fields($table_name) {
		$sql_fields = [
			'system_setting' => "`system_setting`.`flash_sale` as 'flash_sale', if(`system_setting`.`date`,date_format(`system_setting`.`date`,'%m/%d/%Y'),'') as 'date', `system_setting`.`just_sold` as 'just_sold', if(`system_setting`.`js_date`,date_format(`system_setting`.`js_date`,'%m/%d/%Y'),'') as 'js_date', `system_setting`.`recommended` as 'recommended', if(`system_setting`.`r_date`,date_format(`system_setting`.`r_date`,'%m/%d/%Y'),'') as 'r_date', `system_setting`.`latest_product` as 'latest_product', if(`system_setting`.`lp_date`,date_format(`system_setting`.`lp_date`,'%m/%d/%Y'),'') as 'lp_date', `system_setting`.`prods_1` as 'prods_1', if(`system_setting`.`p1_date`,date_format(`system_setting`.`p1_date`,'%m/%d/%Y'),'') as 'p1_date', `system_setting`.`id` as 'id', `system_setting`.`prods_2` as 'prods_2', if(`system_setting`.`p2_date`,date_format(`system_setting`.`p2_date`,'%m/%d/%Y'),'') as 'p2_date', `system_setting`.`prods_3` as 'prods_3', if(`system_setting`.`p3_date`,date_format(`system_setting`.`p3_date`,'%m/%d/%Y'),'') as 'p3_date', `system_setting`.`location` as 'location', `system_setting`.`phone` as 'phone', `system_setting`.`email` as 'email', `system_setting`.`system_bg` as 'system_bg', `system_setting`.`hidder` as 'hidder', `system_setting`.`unhidder` as 'unhidder', `system_setting`.`new_script` as 'new_script', `system_setting`.`new_script2` as 'new_script2', `system_setting`.`new_script3` as 'new_script3', `system_setting`.`new_script4` as 'new_script4', `system_setting`.`new_script5` as 'new_script5', `system_setting`.`new_script6` as 'new_script6', `system_setting`.`new_script7` as 'new_script7', `system_setting`.`new_script8` as 'new_script8'",
			'supplier' => "`supplier`.`id` as 'id', `supplier`.`name` as 'name', `supplier`.`shop_id` as 'shop_id'",
			'products' => "`products`.`id` as 'id', `products`.`promote_class` as 'promote_class', `products`.`promote_code` as 'promote_code', `products`.`slug` as 'slug', `products`.`name` as 'name', `products`.`seller_sku` as 'seller_sku'",
			'details' => "`details`.`id` as 'id', `details`.`payment` as 'payment', `details`.`supp_conf` as 'supp_conf', `details`.`reference_number` as 'reference_number', `details`.`product_name` as 'product_name', `details`.`supp_name` as 'supp_name'",
			'category' => "`category`.`id` as 'id', `category`.`name` as 'name', `category`.`cat_ban` as 'cat_ban'",
		];
	...
	function htmlUserBar() {
		...
		<nav class="navbar navbar-default navbar-fixed-top hidden-print" role="navigation">
			<div class="navbar-header">
				...
				<a class="navbar-brand" href="<?php echo PREPEND_PATH; ?>index.php"><i class="glyphicon glyphicon-home"></i> Tsavo manager</a>
			</div>
