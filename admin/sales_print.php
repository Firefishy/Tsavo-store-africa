<?php
	...
	function generateRow($from, $to, $conn){
		...
		$stmt = $conn->prepare("SELECT *, sales.id AS salesid FROM sales LEFT JOIN users ON users.id=sales.user_id WHERE sales_day BETWEEN '$from' AND '$to' ORDER BY sales_day DESC");
		$stmt->execute();
		$total = 0;
		foreach($stmt as $row){
			...
			<tr>
				...
				<td>'.$row['pay_id'].'</td>
				<td align="right">Kshs. '.number_format($amount, 2).'</td>
			</tr>
			';
		}
