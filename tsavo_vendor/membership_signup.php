<?php
	$app_name = 'TSAVO POINT OF SALE SYSTEM';
		...
		<div class="col-sm-12 login-container">
			<div class="">
				...
				<div class="panel-heading">
					<h1 class="login-title"><strong><?php echo $Translation['sign up here']; ?></strong></h1>
				</div>

				<div class="panel-body">
					<form method="post" action="membership_signup.php">
						<div class="input-group">
							<label for="username" style="color:#FEBFFF;"><?php echo $Translation['username']; ?></label>
						...
						<div class="row">
							<div class="col-sm-6">
								<div class="input-group">
									<label for="password" style="color:#FEBFFF;"><?php echo $Translation['password']; ?></label>
							...
							<div class="col-sm-6">
								<div class="input-group">
									<label for="confirmPassword" style="color:#FEBFFF;"><?php echo $Translation['confirm password']; ?></label>
						...
						<div class="input-group">
							<label for="email" style="color:#FEBFFF;"><?php echo $Translation['email']; ?></label>
							<input class="form-control" type="text" required="" placeholder="<?php echo $Translation['email']; ?>" id="email" name="email">
						</div>

						<div class="input-group">
							<label for="group" style="color:#FEBFFF;"><?php echo $Translation['group']; ?></label>
								...
								for($cf = 1; $cf <= 4; $cf++) {
									if($adminConfig['custom'.$cf] != '') {
										...
										<div class="row input-group">
										   <div class="col-sm-3"><label style="color:#FEBFFF;" for="custom<?php echo $cf; ?>"><?php echo $adminConfig['custom'.$cf]; ?></label></div>
	...
	<script>
		$j(function() {
			...
			$j('#password').on('keyup blur', function() {
				...
				if(ps == 'strong') {
					$j('#password').parents('.input-group').removeClass('has-error has-warning').addClass('has-success');
					$j('#password').attr('title', '<?php echo html_attr($Translation['Password strength: strong']); ?>');
				} else if(ps == 'good') {
					$j('#password').parents('.input-group').removeClass('has-success has-error').addClass('has-warning');
					$j('#password').attr('title', '<?php echo html_attr($Translation['Password strength: good']); ?>');
				} else {
					$j('#password').parents('.input-group').removeClass('has-success has-warning').addClass('has-error');
			...
			$j('#confirmPassword').on('keyup blur', function() {
				if($j('#confirmPassword').val() != $j('#password').val() || !$j('#confirmPassword').val().length) {
					$j('#confirmPassword').parents('.input-group').removeClass('has-success').addClass('has-error');
				} else {
					$j('#confirmPassword').parents('.input-group').removeClass('has-error').addClass('has-success');
				}
			});
			...
			$j('#email').on('change', function() {
				if(validateEmail($j('#email').val())) {
					$j('#email').parents('.input-group').removeClass('has-error').addClass('has-success');
				} else {
					$j('#email').parents('.input-group').removeClass('has-success').addClass('has-error');
				}
			});
		...
		function jsValidateSignup() {
			...
			/* user exists? */
			if(!$j('#username').parents('.input-group').hasClass('has-success')) {
